# A Dockerfile for debugging the Open Modeling Framework locally
FROM ubuntu:24.04
LABEL maintainer=<david.pinney@nreca.coop>

# Install and setup OMF reqs
RUN apt-get -y update && apt-get install -y sudo vim python3 python3-pip python3-setuptools
RUN mkdir /home/omf
...
RUN cd /home/omf/; python3 -m pip install . --break-system-packages
...
# - Build image with command `docker build . -f omf.dockerfile -t omfim`
